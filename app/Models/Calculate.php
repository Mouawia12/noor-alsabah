...
public function scopesumspendcountdesc($query, $calculate_month_m, $calculate_month_y, $shop_id,$manager_id,$from,$to)
{
   ...
   COALESCE(count(cd.calculate_detail_id), 0) as count_statement,m.manager_name,
   COALESCE(sum(cd.calculate_month_pay), 0) as sum_det_calculate_month_pay,

    (p.calculate_month_val - COALESCE(sum(cd.calculate_month_pay), 0)) as xx

        FROM   calculate p
        left join  calculate_detail cd on p.calculate_id=cd.calculate_id
        left join  shop w on p.shop_id=w.shop_id
  ...
  ";
    ...
    public function scopeserachspendcountdesc($query, $calculate_month_m, $calculate_month_y, $shop_id, $manager_id , $from , $to)
    {
        $calculate_month_m = TRIM($calculate_month_m);
        $calculate_month_y = TRIM($calculate_month_y);
        $from = TRIM($from);
...
left join  manager m on s.manager_id=m.manager_id
    ...
    public function scopeserachspenddatadesc($query, $calculate_month_m, $calculate_month_y, $shop_id, $manager_id, $from, $to)
    {
        $a = $_POST['length'];
        $b = $_POST['start'];
        $calculate_month_m = TRIM($calculate_month_m);
