...
class WorkersController extends Controller
{
    ...
    {
        if (Perm::get_function_access(72)) {
            $arr_per_nation = DB::table('nation')->select('nation_name_ar')->distinct()->pluck('nation_name_ar');
            $arr_per_nation = $arr_per_nation->toArray();
            $arr_per_job = DB::table('job')->select('job_name')->distinct()->pluck('job_name');
            $arr_per_job = $arr_per_job->toArray();
            $attributeNames = array(
                'uploadFile.*' => 'نوع الملف',
            );
            $validator = Validator::make($request->all(), [
                'uploadFile.*' => ['required', 'file', 'mimes:csv,txt,xlx,xlsx,xls,pdf,ppt,pptx,doc,docx,xlsx,jpg,jpeg,bmp,png,rtf,zip'],
            ]);
            ...
            if ($validator->fails()) {
                ...
                if ($request->hasFile('uploadFile')) {
                    ...
                    $request->uploadFile->move(public_path('uploads/users/images/'), $imageName);
                    $inputFileName = 'uploads/users/images/' . $imageName;
                }
                ...
                $flag = true;
                $i = 0;
                $message_err = 'خطأ في تنسيق الملف ';
                $flag = 0;
                for ($x = 2; $x <= count($allDataInSheet); $x++) {
                    ...
                    $col_range = $allDataInSheet[$x]["B"];
                        $fam[$i]['ssn'] = isset($allDataInSheet[$x]['A']) ? trim($allDataInSheet[$x]['A']) : '';
                        $fam[$i]['worker_name'] = isset($allDataInSheet[$x]['B']) ? trim($allDataInSheet[$x]['B']) : '';
                        $fam[$i]['nation'] = isset($allDataInSheet[$x]['C']) ? trim($allDataInSheet[$x]['C']) : '';
                        $fam[$i]['job'] = isset($allDataInSheet[$x]['D']) ? trim($allDataInSheet[$x]['D']) : '';
                        $fam[$i]['passport'] = isset($allDataInSheet[$x]['E']) ? trim($allDataInSheet[$x]['E']) : '';
                        $fam[$i]['end_passport'] = isset($allDataInSheet[$x]['F']) ? trim($allDataInSheet[$x]['F']) : '';
                        $fam[$i]['start_can'] = isset($allDataInSheet[$x]['G']) ? trim($allDataInSheet[$x]['G']) : '';
                        $fam[$i]['end_can'] = isset($allDataInSheet[$x]['H']) ? trim($allDataInSheet[$x]['H']) : '';
                        $fam[$i]['dob'] = isset($allDataInSheet[$x]['I']) ? trim($allDataInSheet[$x]['I']) : '';
                        $fam[$i]['outside'] = isset($allDataInSheet[$x]['J']) ? trim($allDataInSheet[$x]['J']) : '';
                        $fam[$i]['end_can_hij'] = isset($allDataInSheet[$x]['K']) ? trim($allDataInSheet[$x]['K']) : '';
                        $fam[$i]['mobile'] = isset($allDataInSheet[$x]['L']) ? trim($allDataInSheet[$x]['L']) : '';





$dateTime = DateTime::createFromFormat('Y-m-d', $fam[$i]['end_passport']);
$errors = DateTime::getLastErrors();
if (!empty($errors['warning_count']  and date('Y-m-d', strtotime($fam[$i]['end_passport'])) != $fam[$i]['end_passport'])) {
// $result['flage'] = 1;
// $result['status'] = false;
// $message_err = 'تأكد من ان hg[ksdm موجود في sssssssssss ' . $fam[$i]['end_passport'];
$result_2 = 3;
$flag = 1;
$result['flage'] = 1;
$result['status'] = false;
$message_err = 'تأكد من تاريخ انتهاء الجواز مكتوب بالصيغة الصحيحة ' .'Y-m-d   مثل 1995-07-13    ' .$fam[$i]['end_passport'];
$message_imp = 'تأكد من تاريخ انتهاء الجواز مكتوب بالصيغة الصحيحة ' .'Y-m-d   مثل 1995-07-13    '.$fam[$i]['end_passport'];
$result['message_err'] =  $message_err;
$result['message_imp'] =  $message_imp;
...
$dateTime = DateTime::createFromFormat('Y-m-d', $fam[$i]['start_can']);
$errors = DateTime::getLastErrors();
if (!empty($errors['warning_count'] and date('Y-m-d', strtotime($fam[$i]['start_can'])) != $fam[$i]['start_can'])) {
$result_2 = 3;
$flag = 1;
$result['flage'] = 1;
$result['status'] = false;
$message_err = 'تأكد من تاريخ انتهاء الاقامة مكتوب بالصيغة الصحيحة ' .'Y-m-d   مثل 1995-07-13    ' .$fam[$i]['start_can'];
$message_imp = 'تأكد من تاريخ انتهاء الاقامة مكتوب بالصيغة الصحيحة ' .'Y-m-d   مثل 1995-07-13    '.$fam[$i]['start_can'];
$result['message_err'] =  $message_err;
$result['message_imp'] =  $message_imp;
...
$dateTime = DateTime::createFromFormat('Y-m-d', $fam[$i]['end_can']);
$errors = DateTime::getLastErrors();
if (!empty($errors['warning_count'] and date('Y-m-d', strtotime($fam[$i]['end_can'])) != $fam[$i]['end_can'])) {
$result_2 = 3;
$flag = 1;
$result['flage'] = 1;
$result['status'] = false;
$message_err = 'تأكد من تاريخ اصدار الاقامة مكتوب بالصيغة الصحيحة ' .'Y-m-d   مثل 1995-07-13    ' .$fam[$i]['end_can'];
$message_imp = 'تأكد من تاريخ اصدار الاقامة مكتوب بالصيغة الصحيحة ' .'Y-m-d   مثل 1995-07-13    '.$fam[$i]['end_can'];
$result['message_err'] =  $message_err;
$result['message_imp'] =  $message_imp;
...
$dateTime = DateTime::createFromFormat('Y-m-d', $fam[$i]['dob']);
$errors = DateTime::getLastErrors();
//

//echo date('Y-m-d', strtotime($fam[$i]['dob'])) ;
if (!empty($errors['warning_count'] and date('Y-m-d', strtotime($fam[$i]['dob'])) != $fam[$i]['dob'])) {
$result_2 = 3;
$flag = 1;
$result['flage'] = 1;
$result['status'] = false;
$message_err = 'تأكد من تاريخ الميلاد مكتوب بالصيغة الصحيحة ' .'Y-m-d   مثل 1995-07-13    ' .$fam[$i]['dob'];
$message_imp = 'تأكد من تاريخ الميلاد مكتوب بالصيغة الصحيحة ' .'Y-m-d   مثل 1995-07-13    '.$fam[$i]['dob'];
$result['message_err'] =  $message_err;
$result['message_imp'] =  $message_imp;
...
break;
                        ...
                        $nation_true = '';
                        if ($fam[$i]['nation'] != '') {
                            $array_nation = $fam[$i]['nation'];
                            $arr_per_job = DB::table('job')->select('job_name')->distinct()->pluck('job_name');
                            $arr_per_job = $arr_per_job->toArray();

                            if (in_array($array_nation, $arr_per_nation) == false) {
                                $arr_per_nation = DB::table('nation')->select('nation_name_ar')->distinct()->pluck('nation_name_ar');
                                $arr_per_nation = $arr_per_nation->toArray();
                                $nation_true = DB::table('nation')->insertGetId(['nation_name_ar' => $array_nation]);
                                $fam[$i]['nation'] = $nation_true;
                                $result['flage'] = 0;
                                $result['status'] = true;

                            } else if (in_array($array_nation, $arr_per_nation) == true) {
                                $nation_true = DB::table('nation')->select('nation_id')->where('nation_name_ar', $fam[$i]['nation'])->first();
                                $nation_true = $nation_true->nation_id;
                                if ($nation_true == '') {
                                    $result['flage'] = 1;
                                    $result['status'] = false;
                                    $message_err = 'تأكد من ان hg[ksdm موجود في الثوابت ' . $array_nation;
                                } else {
                                    $fam[$i]['nation'] = $nation_true;
                                    $result['flage'] = 0;
                                    $result['status'] = true;
                            ...
                            $result['flage'] = 1;
                            $result['status'] = false;
                            $message_err = 'تأكد من جميع المستوى مدخلة ';
                        }
                        $fam[$i]['nation'] = $nation_true;
                        $job_true = '';
                        if ($fam[$i]['job'] != '') {
                            $array_job = $fam[$i]['job'];
                            if (in_array($array_job, $arr_per_job) == false) {
                                $job_true = DB::table('job')->insertGetId(['job_name' => $array_job]);
                                $fam[$i]['job'] = $job_true;
                                $result['flage'] = 0;
                                $result['status'] = true;
                            } else if (in_array($array_job, $arr_per_job) == true) {
                                $job_true = DB::table('job')->select('job_id')->where('job_name', $fam[$i]['job'])->first();
                                $job_true = $job_true->job_id;
                                if ($job_true == '') {
                                    $result['flage'] = 1;
                                    $result['status'] = false;
                                    $message_err = 'تأكد من ان hg[ksdm موجود في الثوابت ' . $array_job;
                                } else {
                                    $fam[$i]['job'] = $job_true;
                                    $result['flage'] = 0;
                                    $result['status'] = true;
...
// }
                ...
                if ($result['flage'] == 0) {
                    ...
                    foreach ($fam as $d) {
                        ...
                        $passport = $d['passport'];
                        $end_passport = $d['end_passport'];
                        $start_can = $d['start_can'];
                        $end_can = $d['end_can'];
                        $dob = $d['dob'];
                        $outside = $d['outside'];
                        if (trim($outside) == 'نعم') {
                        ...
                        $end_can_hij = $d['end_can_hij'];
                        $mobile = $d['mobile'];
                        $job = $d['job'];
                        $is_find = DB::table('workers')->where('ssn', $ssn)->count();
                        if ($is_find != '0') {
                            ...
                            $result2 = DB::table('workers')
                                ...
                                ->update([
                                    ...
                                    'dop' =>  Carbon::parse($end_passport)->format('Y-m-d'),
                                    'dos' =>  Carbon::parse($start_can)->format('Y-m-d'),
                                    'doe' =>  Carbon::parse($end_can)->format('Y-m-d'),
                                    'dob' =>   Carbon::parse($dob)->format('Y-m-d'),
                                    'inside' => $outside,
                                    'end_can_hij' => $end_can_hij,
                                    'phone' => $mobile,
                                    'job_id' => $job,
                                    'is_imp' => 1,
                            ...
                            $result_eee2 = DB::table('workers')->insertGetId([
                                'worker_name' => $worker_name,
                                'ssn' => $ssn,
                                'nation_id' => $nation,
                                'passport_no' => $passport,
                ...
                if ($result_2 == 1) {
                    $result['status'] = true;
                    $result['message_out'] = "تم حفظ البيانات بنجاح";
                } else if ($result_2 == 3) {
                    $result['status'] = false;
                    $result['message_out'] = $result['message_imp'];
                } else {
                    $result['status'] = false;
                    $result['message_out'] = $message_err;
                }
            }
                ...
                $row[] = '<div class="d-flex align-items-center">
																<div class="me-2 position-relative">
																	<div class="symbol symbol-35px symbol-circle">
																		<img alt="" src="999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999' . $avatar . '" style="cursor: pointer;" onclick="this.style.width=\'500px\'; this.style.height=\'auto\'; this.insertAdjacentHTML(\'afterend\', \'<span style=\"cursor: pointer; color: red; position: absolute; top: 0; right: 0;\" onclick=\"this.previousElementSibling.style.width=\'auto\'; this.previousElementSibling.style.height=\'auto\'; this.remove();\">×</span>\');">
																	</div>
																</div>
...
dd($worker_health);*/
    ...
    public function store___________(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'ssn.required' => 'رقم الإقامة  مطلوب',
