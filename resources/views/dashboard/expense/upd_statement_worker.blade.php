...
<form id="upd_statement_data" name="upd_statement_data" class="form" action="{{ route('dashboard.expense.updstatement') }}"
    ...
    <input name="expense_id_db" id="expense_id_db" value="{{ $expense->expense_id }}" im-insert="true"
        ...
        <input name="worker_id" id="worker_id" value="{{ $expense->worker_id }}" im-insert="true"
        data-inputmask="'alias' : 'integer' " type="text" style="display:none"
                ...
                <div class="row p-0 mb-0 px-1">
                    <div class="col">
                        <div class="border border-dashed border-dark text-center min-w-125px rounded pt-4 pb-2 my-3">
                            <span class="fs-6 fw-bold text-danger ">اسم العامل :</span> <span class=" fw-bold text-dark ">{{ $worker->worker_name}}</span> 														
                        </div>
                    </div>
                    <div class="col">
                        <div class="border border-dashed border-dark text-center min-w-125px  rounded pt-4 pb-2 my-3">
                            <span class="fs-6 fw-bold text-danger ">شهر الدفع:</span> <span class=" fw-bold text-dark ">{{ $expense->expense_month_desc}}</span> 														
                        </div>
                    </div>
                </div>
...
function chk_expense_worker(month,year) {
    let  worker_id  = $('#worker_id').val();
...
var year=year_val;
    ...
    if(worker_id!=''){
$.ajax({
url: " {{ route('dashboard.general.chk_expense_worker') }}",
dataType: 'json',
type: 'POST',
async: false,
'data': {
...
$(document).ready(function() {
    ...
    onChange: function(selectedDates, dateStr, instance) {
        chk_expense_worker(selectedDates[0].getMonth() + 1,selectedDates[0].getFullYear())
  },
