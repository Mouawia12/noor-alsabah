...
@section('content')
    ...
    <form id="save_expensecategoty" name="save_expensecategoty" class="form" action="{{ route('dashboard.constant.storeexpensecategoty') }}"
                ...
                <div class="card">
                    <div class="card-body px-1">
                        <div class="alert alert-dismissible   d-flex flex-column flex-sm-row w-100 p-5 mb-6"
                            id="errorBox_expensecategoty" style="display: none !important">
                            ...
                            <div class="d-flex flex-column text-light pe-0 pe-sm-10">
                                <span id="displayErrors_expensecategoty" class="mb-2  fw-bolder text-light"></span>
                            </div>
                            <button type="button"
                        ...
                        <div class="mb-0">
                            ...
                            <div class=" col-12 col-lg-4 col-md-12 col-sm-12 mb-5"><label for="expense_categoty_name"
                                    class="form-label required fs-6 fw-bold text-dark mb-3">التصنيف</label>
                                <div class="input-group">
                                    <div class="input-group-prepend"><span class="input-group-text"><i
                                                class="fas fa-user-tie fa-fw text-dark "></i></span></div><input
                                        type="text" name="expense_categoty_name" id="expense_categoty_name"
                                        class="form-control fw-bolder  text-dark" placeholder="التصنيف" value=""
                                        autocomplete="off">
                                </div>
                            </div>
                    ...
                    <div id="result_expensecategoty_tbl" name="result_expensecategoty_tbl"></div>
...
<script>
    view_all_expensecategoty("{{ route('dashboard.constant.tblexpensecategoty') }}");

    function del_expensecategoty(id) {
        swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{{ route('dashboard.constant.delexpensecategoty') }}",
                    'type': 'POST',
                    'dataType': 'json',
                    'async': false,
                    'data': {
