var KTFormControls = function () {
    ...
    var save_accountings_valid = function () {
        $("#save_accountings").validate({
    rules: {
        worker_id: {
            required: true
        },
    ...
    submitHandler: function (form) {
                var formData = new FormData($("#save_accountings")[0]);
        save_accountings(formData);
        return false;
    }
});
};
var upd_accountings_data_valid = function () {
$("#upd_accountings_data").validate({
    ignore: ":hidden",
    rules: {
        accountings_month_desc: {
            required: true
        },
        accountings_month_val: {
            required: true
        },
    },
     highlight: function (element, errorClass, validClass) {
    ...
    var save_paymentsmonth_valid = function () {
                $("#save_paymentsmonth").validate({
            rules: {
                payments_month_desc: {
                    required: true
                },
            ...
            invalidHandler: function (event, validator) {},
            submitHandler: function (form) {
                        var formData = new FormData($("#save_paymentsmonth")[0]);
                save_paymentsmonth(formData);
                return false;
            }
        });
    };
    var upd_payments_data_valid = function () {
        $("#upd_payments_data").validate({
            ignore: ":hidden",
            rules: {
                payments_month_desc: {
                    required: true
 ...
 STATUS_IN: {
    ...
    return {
        init: function () {
            save_paymentsmonth_valid();
            upd_payments_data_valid();
            save_accountings_valid();
            upd_accountings_data_valid();
...
$("#errorBox_accountings").show();
$("#displayErrors_accountings").html('');
$("#errorBox_accountings").removeClass("bg-success");
$("#errorBox_accountings").addClass( "bg-danger" );
$.each(resp.message, function (key, item)
{
$('#displayErrors_accountings').append('<p>'+item+'</p');
});
if(resp.message_out!=''){
    $('#displayErrors_accountings').append('<p>'+resp.message_out+'</p');
...
$("#errorBox_accountings").show();
$("#displayErrors_accountings").html('');
$("#errorBox_accountings").removeClass("bg-danger");
$("#errorBox_accountings").addClass("bg-success");
$("#displayErrors_accountings").html(resp.message_out);
document.documentElement.scrollTop = 0;
$("#save_accountings")[0].reset();
unload_message();

$('#save_accountings').children().find('input,select,file').each(function(){
...
function upd_accountings(id) {
    $(document).ready(function () {
    //load_message();
    var url = $('.upd_accountings').data('url');
    $.ajax({
    url:url,
    'type': 'POST',
    async: false,
    ...
    $("#upd_accountings_data").submit(function (e) {
    e.preventDefault();
    var formData = new FormData($("#upd_accountings_data")[0]);
    if ($('#upd_accountings_data').valid()) {
    upd_accountings_data_send(formData);
    } else {
    var alert = $('#kt_form_1_msg');
    alert.removeClass('kt--hide').show();
    ...
    function upd_accountings_data_send(formData) {

    formData.append('X-CSRF-TOKEN', '{{ csrf_token }}');
    $.ajax({
    url: $("#upd_accountings_data").attr('action'),
    dataType: 'json',
    type: 'POST',
    async: false,
    data: formData,
...
function view_all_accountings() {
    ...
    async: false,
    data: {
        payments_month_desc:payments_month_desc,
        worker_id:worker_id,
    ...
    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
    success: function (data) {
    var container = $('#result_accountings_tbl');
    if (data) {
    container.html(data);
    //unload_message();
    }
...
function print_paymentsmonth(id) {
    $(document).ready(function () {
  // load_message();
   alert(id);
    var url = $('.print_paymentsmonth').data('url');
    $.ajax({
    url:url,
    dataType: 'binary',
    'type': 'POST',
...
function upd_pmonth(id) {
    ...
    $("#upd_payments_data").submit(function (e) {
    e.preventDefault();
    var formData = new FormData($("#upd_payments_data")[0]);
    if ($('#upd_payments_data').valid()) {
    upd_payments_data_send(formData);
    } else {
    var alert = $('#kt_form_1_msg');
    alert.removeClass('kt--hide').show();
    $("#view_prim_const_m").scrollTop(0);
    ...
    function upd_payments_data_send(formData) {

    formData.append('X-CSRF-TOKEN', '{{ csrf_token }}');
    $.ajax({
    url: $("#upd_payments_data").attr('action'),
    dataType: 'json',
    type: 'POST',
    async: false,
    data: formData,
    ...
    success: function (resp) {
    if (resp.status == false) {
        $("#errorBox_paymentsmonth").show();
        $("#displayErrors_paymentsmonth").html('');
        $("#errorBox_paymentsmonth").removeClass("bg-success");
        $("#errorBox_paymentsmonth").addClass( "bg-danger" );
        $.each(resp.message, function (key, item)
        {
        $('#displayErrors_paymentsmonth').append('<p>'+item+'</p');
        });
    $('#displayErrors_paymentsmonth').append('<p>'+resp.message_out+'</p');
    document.documentElement.scrollTop = 0;
    } else {
        $("#view_prim_const_m").modal('hide');
    view_all_pmonth();
    $("#displayErrors_paymentsmonth").html('');
    $("#errorBox_paymentsmonth").removeClass("bg-danger");
    $("#errorBox_paymentsmonth").addClass("bg-success");
    $("#displayErrors_paymentsmonth").html(resp.message_out);
...
function save_paymentsmonth(formData) {
	load_message();
formData.append('X-CSRF-TOKEN', '{{ csrf_token }}');
 /* var xxxxx= $('input[name=PROJECT_DESCRIPTION_IN]').val();
  alert(xxxxx);*/
$.ajax({
url: $("#save_paymentsmonth").attr('action'),
dataType: 'json',
type: 'POST',
...
$("#displayErrors_paymentsmonth").html(resp.message_out);
document.documentElement.scrollTop = 0;
$("#save_paymentsmonth")[0].reset();
unload_message();

$('#save_paymentsmonth').children().find('input,select,file').each(function(){
$(this).val('').trigger('change');
});
