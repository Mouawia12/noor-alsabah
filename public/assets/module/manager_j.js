...
var KTFormControls = function () {
    ...
    var save_manager_valid = function () {
                $("#save_manager").validate({
            rules: {
                manager_name: {
                    required: true
                },
            ...
            submitHandler: function (form) {
                        var formData = new FormData($("#save_manager")[0]);
                save_manager(formData);
                return false;
            }
        });
    };
    var upd_manager_data_valid = function () {
        $("#upd_manager_data").validate({
            ignore: ":hidden",
            rules: {
                manager_name: {
                    required: true
...
function print_manager(id) {
    $(document).ready(function () {
  // load_message();
    var url = $('.print_manager').data('url');
    $.ajax({
    url:url,
    dataType: 'binary',
    'type': 'POST',
...
function upd_manager(id) {
    $(document).ready(function () {
    //load_message();
    var url = $('.upd_manager').data('url');
    $.ajax({
    url:url,
    'type': 'POST',
    async: false,
    ...
    $("#upd_manager_data").submit(function (e) {
    e.preventDefault();
    var formData = new FormData($("#upd_manager_data")[0]);
    if ($('#upd_manager_data').valid()) {
    upd_manager_data_send(formData);
    } else {
    var alert = $('#kt_form_1_msg');
    alert.removeClass('kt--hide').show();
    ...
    function upd_manager_data_send(formData) {

    formData.append('X-CSRF-TOKEN', '{{ csrf_token }}');
    $.ajax({
    url: $("#upd_manager_data").attr('action'),
    dataType: 'json',
    type: 'POST',
    async: false,
    data: formData,
    ...
    if (resp.status == false) {
        $("#errorBox_manager").show();
        $("#displayErrors_manager").html('');
        $("#errorBox_manager").removeClass("bg-success");
        $("#errorBox_manager").addClass( "bg-danger" );
        $.each(resp.message, function (key, item)
        {
        $('#displayErrors_manager').append('<p>'+item+'</p');
        });
    $('#displayErrors_manager').append('<p>'+resp.message_out+'</p');
    document.documentElement.scrollTop = 0;
    } else {
        $("#view_prim_const_m").modal('hide');
    view_all_manager();
    $("#displayErrors_manager").html('');
    $("#errorBox_manager").removeClass("bg-danger");
    $("#errorBox_manager").addClass("bg-success");
    $("#displayErrors_manager").html(resp.message_out);
...
function view_all_manager() {
    ...
    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
    success: function (data) {
    var container = $('#result_manager_tbl');
    if (data) {
    container.html(data);
    //unload_message();
    }
...
$("#displayErrors_manager").html(resp.message_out);
document.documentElement.scrollTop = 0;
$("#save_manager")[0].reset();
unload_message();

$('#save_manager').children().find('input,select,file').each(function(){
